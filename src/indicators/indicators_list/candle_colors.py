...
#
# def calculate_candle_colors(df, indicator_color='StDev'):
#
...
def calculate_candle_colors(df, indicator_color='StDev', custom_params=None):
    ...
    """
    # Default parameters for all supported indicators
    default_params = {
        'ZScore': {
            'centreline': 'peaks_valleys_avg',
            'peaks_valleys_params': {'periods': 20, 'max_aVWAPs': None},
    ...
    def map_TTM_squeeze(row):
        return colors['orange'] if row['TTM_squeeze_Active'] == 1 else colors['black']

    # ... (include other mapping functions as needed) ...
    ...
    # Apply the selected color mapping
    base_indicator = indicator_color.split('_color')[0] if '_color' in indicator_color else indicator_color
    if base_indicator in color_mappers:
        return {'color': color_mappers[base_indicator](df)}
