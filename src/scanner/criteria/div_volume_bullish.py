...
def div_volume_bullish(
    ...
    # 1. Find the most recent Volume bullish divergence
    divergence_indices = []
    
    # Check only Volume divergence columns
    vol_bullish_columns = [
        'Vol_Regular_Bullish', 
        'Vol_Hidden_Bullish'
    ]
    
    # Get the index of the most recent Volume bullish divergence
    ...
    if not divergence_indices:
        return pd.DataFrame()  # No Volume divergence found
    
    # Get the most recent Volume divergence
    most_recent_divergence_idx = max(divergence_indices)
    divergence_row = df.loc[most_recent_divergence_idx]

    # 2. Check if any Volume bearish divergence happened AFTER the bullish one
    vol_bearish_columns = [
        'Vol_Regular_Bearish',
        'Vol_Hidden_Bearish'
    ]
    ...
    # 4. Return the latest row if the most recent Volume divergence was bullish and still valid
...
def calculate_indicator(df, **params):
    return div_Volume_bullish(df, **params)
