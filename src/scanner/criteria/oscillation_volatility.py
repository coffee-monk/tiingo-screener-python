...
#     df: pd.DataFrame,
#     cross_count: int = 5,
#     avg_deviation: float = 0.5,
#     oscillation_score: float = 3.0
# ) -> pd.DataFrame:
#     """
#     Scan for strong oscillation volatility signals.
#    
#     Parameters:
#         - cross_count (int): Minimum MA crosses in lookback (default: 5)
#         - avg_deviation (float): Minimum std dev from MA (default: 0.5)
#         - oscillation_score (float): Min score (count*deviation) (default: 3.0)
#    
#     Returns:
...
#     # Main conditions
#     conditions = (
#         (latest['MA_Cross_Count'] >= cross_count) and
#         (latest['MA_Avg_Deviation_Z'] >= avg_deviation) and
#         (latest['MA_Oscillation_Score'] >= oscillation_score)
#     )
#    
#     return df.iloc[-1:].copy() if conditions else pd.DataFrame()
#
