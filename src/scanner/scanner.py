...
#             result_row = {
#                 'date': df.index[-1],  # Using date instead of Timestamp
#                 'Ticker': ticker,
#                 'Timeframe': timeframe,
...
#                           api_key)
#
# def _advanced_scan(timeframe_criteria, logic='AND', api_key='Tiingo API Key'):
#     """
#     Enhanced timeframe scanner supporting multiple criteria per timeframe.
#     Only passes if:
#     1. All specified timeframes exist
#     2. For each timeframe, ALL criteria pass
#     3. Between timeframes: applies specified logic ('AND' or 'OR')
#     """
#     # First validate and load all criteria functions
#     timeframe_configs = {}
...
#             df = _load_indicator_file(INPUT_DIR / files[timeframe])
#            
#             # Check ALL criteria for this timeframe
#             passed_all = True
#             combined_results = None
#            
#             for criteria_func in criteria_funcs:
...
#                 combined_results['Ticker'] = ticker
#                 combined_results['Timeframe'] = timeframe
#                 timeframe_results[timeframe] = combined_results
#
#         # Apply logic between timeframes
#         if logic == 'AND' and all(timeframe_signals.values()):
#             # Convert set intersection to list for pandas compatibility
#             common_dates = list(set.intersection(*[
#                 set(r.index) for r in timeframe_results.values()
#             ])) if timeframe_results else []
#            
#             if common_dates:
#                 # Combine all criteria results for common dates
#                 combined = pd.concat([
#                     r.loc[common_dates] for r in timeframe_results.values()
#                 ], axis=1)
#                 all_results.append(combined)
#                
#         elif logic == 'OR' and any(timeframe_signals.values()):
#             # For OR logic, include all passing timeframes
...
#
# def _process_results(results, scan_type, api_key='Tiingo API Key'):
#     """Process results using date column consistently"""
...
#         if col not in minimal_results.columns and col not in ['date', 'Ticker', 'Timeframe', 'Close']:
...
#
# def _load_indicator_file(filepath):
#     """Load indicator file keeping date as a column"""
...
def _advanced_scan(timeframe_criteria, logic='AND', api_key='Tiingo API Key'):
    ...
    for ticker, files in ticker_files.items():
        ...
        # Apply logic between timeframes
        if logic == 'AND' and all(timeframe_signals.values()):
            # For AND logic, we need matching dates across timeframes
...
def _attach_fundamentals_to_scanner(scanner_df, api_key):
    """Attach fundamental data to results"""
    if 'Ticker' not in scanner_df.columns:
        return scanner_df
