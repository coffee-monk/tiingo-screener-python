...
>>>>>>> cea85ccf94338816b4209ff9b153f0e3f3b7f983
    if padding_ratio > 0 and len(df) > 0:
        ...
        last_date = last_candle['date']
        tf = str(timeframe).lower()
        tf_mapping = {'1min':'1min','5min':'5min','15min':'15min','30min':'30min',
                     '1h':'1H','1hour':'1H','4h':'4H','4hour':'4H',
                     'd':'1D','day':'1D','daily':'1D','w':'1W','week':'1W','weekly':'1W'}
        freq = tf_mapping.get(tf, '1D')
<<<<<<< HEAD
        future_dates = pd.date_range(
            start=last_date + pd.Timedelta(freq),
    ...
    if not show_volume and 'volume' in df.columns:
        df = df.drop(columns=['volume'])
=======
        future_dates = pd.date_range(start=last_date + pd.Timedelta(freq), periods=padding_candles, freq=freq)
        future_df = pd.DataFrame({'date':future_dates,'open':np.nan,'high':np.nan,'low':np.nan,'close':np.nan,'volume':0})
        indicator_cols = [c for c in df.columns if c.startswith(('aVWAP','OB'))]
        for col in indicator_cols: future_df[col] = last_candle.get(col, np.nan)
        df = pd.concat([df, future_df], ignore_index=True)
    df['date'] = df['date'].dt.strftime('%Y-%m-%d %H:%M:%S')
    if not show_volume and 'volume' in df.columns: df = df.drop(columns=['volume'])
>>>>>>> cea85ccf94338816b4209ff9b153f0e3f3b7f983
    return df, timeframe
...
def configure_base_chart(df, chart):
    ...
    chart.fit()
    chart.candle_style(up_color=colors['teal'], down_color=colors['red'], border_up_color=colors['teal'], 
                      border_down_color=colors['red'], wick_up_color=colors['teal'], wick_down_color=colors['red'])
    chart.grid(False, False)
    chart.price_line(True, False)
...
=======
    chart.volume_config(scale_margin_bottom=0.0, scale_margin_top=0.9, up_color=colors['orange_volume'], down_color=colors['orange_volume'])

def get_charts(df_list):
    num_charts = len(df_list)
    if num_charts < 1 or num_charts > 4: raise ValueError("Input must contain 1-4 DataFrames")
>>>>>>> cea85ccf94338816b4209ff9b153f0e3f3b7f983
    if num_charts == 1:
        main_chart = Chart(inner_width=1.0, inner_height=1.0, maximize=True)
        charts = [main_chart]
...
def add_ui_elements(chart, charts, ticker, timeframe, csv_loader='scanner', show_volume=False):
>>>>>>> cea85ccf94338816b4209ff9b153f0e3f3b7f983
    try:
        if chart.topbar is not None:
           chart.topbar['ticker'].set(ticker)
...
=======
def _load_ticker_csv(charts, key, csv_loader='scanner', show_volume=False):
    from src.visualization.subcharts import CURRENT_SCAN_FILE
    KEY_MAPPINGS = {'-':0,'=':1,'[':2,']':3}
>>>>>>> cea85ccf94338816b4209ff9b153f0e3f3b7f983
    try:
...
=======
        if csv_loader == 'scanner':
            if not CURRENT_SCAN_FILE or not CURRENT_SCAN_FILE.exists():
                ...
                return
            scanner_df = pd.read_csv(CURRENT_SCAN_FILE)
            scanner_df = scanner_df[(scanner_df['Ticker'].notna()) & (scanner_df['Ticker'] != 'nan')]
            ...
            if not available_tickers:
                print(f"No tickers available for {timeframe} timeframe")
                return
        try: current_index = available_tickers.index(current_ticker)
...
=======
        df = pd.read_csv(indicator_file).rename(columns={'Open':'open','Close':'close','Low':'low','High':'high'})
        df.attrs = {'timeframe':timeframe,'ticker':next_ticker}
>>>>>>> cea85ccf94338816b4209ff9b153f0e3f3b7f983
...
>>>>>>> cea85ccf94338816b4209ff9b153f0e3f3b7f983
        ...
        chart.fit()
        print(f"Loaded {next_ticker} ({timeframe}) from {indicator_file.name}")
    except Exception as e: print(f"Error during ticker cycling: {str(e)}")
...
def _take_screenshot(charts, key, screenshot_dir=None):
    ...
    try:
        ...
        print(f"Failed to take screenshot: {e}")
=======
# ... (remaining functions like _take_screenshot, _on_search, etc. remain unchanged) ...
>>>>>>> cea85ccf94338816b4209ff9b153f0e3f3b7f983
